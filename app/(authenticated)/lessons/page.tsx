...
export default function LessonsPage() {
  ...
  const availableGrades = useMemo(() => {
    ...
    lessons.forEach(lesson => {
      ...
      if (grade) grades.add(grade)   // grade 5 included after import
    })
  ...
  }, [searchHasMore, isSearching, searchOffset, searchQuery, gradeFilter, subjectFilter, showError])
      ...
      >
        ...
        {searchWarming && (
          <div className="mb-2 text-sm text-yellow-700 bg-yellow-50 border border-yellow-200 rounded px-3 py-2">
            Семантичний пошук завантажується — показано базові результати
          </div>
        )}
        ...
        {!isLoading && lessons.length > 0 && (
          <div className="mb-6 flex flex-wrap gap-4">
            ...
            <div className="flex items-center text-sm text-muted-foreground ml-auto">
              {isSearchMode
                ? `Знайдено ${searchTotal}, показано ${searchResults.length}`
                : `Показано ${Math.min((currentPage - 1) * PAGE_SIZE + 1, filteredAndSortedLessons.length)}–${Math.min(currentPage * PAGE_SIZE, filteredAndSortedLessons.length)} з ${filteredAndSortedLessons.length} уроків`
              }
            </div>
          </div>
        )}
        ...
        {(isLoading && !isSearchMode) || (isSearching && searchResults.length === 0) ? (
          ...
          <EmptyState
            ...
            description={`Немає результатів для "${searchQuery}"`}
          />
        ) : !isSearchMode && lessons.length === 0 ? (
          <EmptyState
            icon="📚"
            title="Немає доступних уроків"
            description="Повертайтесь незабаром за новим навчальним контентом!"
          />
          ...
          <>
            ...
            {isSearchMode && searchHasMore && (
              <div className="flex justify-center mt-8">
                ...
                >
                  {isSearching ? "Завантаження..." : "Показати ще"}
                </ButtonEnhanced>
              </div>
            )}
          </>
